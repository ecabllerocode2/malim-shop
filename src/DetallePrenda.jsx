...
function DetallePrenda() {
    ...
    useEffect(() => {
        ...
        if (local) {
            setDatos(local);
            setFotoActual(local.fotos?.[0] || "");
        } else {
            // fetch single doc (una sola lectura)
            fetchProductById(id)
                .then((p) => {
                    if (!mounted) return;
                    setDatos(p);
                    setFotoActual(p.fotos?.[0] || "");
                })
                .catch(() => {
                    if (!mounted) return;
                ...
                <div className="mx-3">
                    <div className="flex flex-row font-montserrat justify-between">
                        ...
                        <div className="text-texto text-sm mr-5 text-center">
                            ...
                            {datos.talla ? datos.talla.map((t, idx) => <p key={idx}>{t}</p>) : <p>Sin talla</p>}
                        </div>
                    </div>
            ...
            <div className="mx-3 lg:h-auto">
                ...
                <div className="overflow-x-auto flex scroll-smooth lg:grid lg:grid-cols-3 lg:w-full lg:gap-4">
                    {categoriaFilter().map((prenda) => (
                        <div key={prenda.id} onClick={() => navigate(`/detallePrenda/${prenda.id}`)} className="h-44 w-24 flex-shrink-0 mr-5 lg:w-full lg:h-full">
                            <img loading="lazy" src={prenda.fotos?.[0]} alt={prenda.prenda} className="h-4/6 w-full object-cover" />
